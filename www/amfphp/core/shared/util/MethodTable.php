...
class MethodTable
{
	 ...
	 * @param $strong If flag set to true returns the parameters of the methods strong typed. Added to support a strong typed service browser.
	 */
	function create($className, $servicePath = NULL, &$classComment, $strong = false){
		...
		{
			//PHP5
			$classMethods = MethodTable::getClassMethodsReflection($sourcePath, $className, $classComment, $strong);
		}
		else
		{
			//PHP4
...
//			if ($strong) {
				$methodTable[$methodName]["arguments"] = MethodTable::getStrongMethodArguments($methodSignature, $params, $paramTypes);
//			} else {
...
//			}
	...
	function getClassMethodsReflection($sourcePath, $className, & $classComment, $strong = false)
	{
			...
			if($reflectionMethod->isPublic() && $method->name[0] != '_' && $method->name != 'beforeFilter')
			{
				...
				else
				{
					...
					foreach($reflectionParameter as $parameter){
						...
						$types[ $parameter->getName() ] = "String";
						$type = "String";
						...
						if ($strong)
						{
							...
							try
							{
								$typeReflection = $parameter->getClass();
							}
							catch (Exception $e)
							{
								$types[ $parameter->getName() ] = "String";
...
//					}
	...
	/**
	 * Returns an array with the arguments of a method strong typed.
	 *
	 * @static
	 * @access private
	 * @param $methodSignature (String)The method's signatureg;
	 * @param $commentParams The comments for each parameter in the docblock
	 * @param $types Array of types for the parameters.  Added to support a strong typed service browser.
	 */
	function getStrongMethodArguments($methodSignature, $commentParams, $types){
		if(strlen($methodSignature) < 2){
			//no arguments, return an empty array
			$result = array();
		}else{
			//clean the arguments before returning them
			$result = MethodTable::cleanStrongArguments(explode(",", $methodSignature), $commentParams, $types);
		}
	 ...
	 * @param $commentParams The parameter comments in the docblock.
	 * @param $types Array of types for the parameters.  Added to support a strong typed service browser.
	 */
	function cleanStrongArguments($args, $commentParams, $types){
		$result = array();
